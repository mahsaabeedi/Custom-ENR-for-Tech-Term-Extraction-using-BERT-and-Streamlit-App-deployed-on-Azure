...
Original file is located at
    https://colab.research.google.com/drive/1o7YWXr7N1_PbFGV_0Mq_sAd0tjIxBfGU
"""
...
len_data=int((len(file))/168)
a1 = np.zeros((len_data,8))
a2 = np.zeros((len_data,1))

bias=np.random.randint(24)+24*np.random.randint(7)
for i in range(len_data):
  a1[i] = file[168*i+bias,0:8]
  a2[i] = file[168*i+bias,0]

b1 = np.zeros((3,8))

train = int((len_data*0.64))-3
valid = int((len_data*0.16))-3

X_Valid = np.zeros((valid+1,3,8))
Y_Valid = np.zeros((valid+1,1))

X_Train = np.zeros((train+1,3,8))
...
tstt = int((len_data*0.2))-3

X_Test = np.zeros((tstt,3,8))
...
model.add(LSTM(32,input_shape=(3,8),return_sequences=False, dropout=0.1, recurrent_dropout=0.1))
#model.add(Dropout(0.1))
model.add(Dense(1, activation='sigmoid'))
model.compile(loss='MAE', optimizer='RMSprop', metrics=['accuracy'])## adam , RMSprop , ADAgrad
print(model.summary())
...
model = Sequential()
model.add(keras.layers.recurrent.SimpleRNN(32,input_shape=(3,8),return_sequences=False))
model.add(Dropout(0.1))
model.add(Dense(1, activation='sigmoid'))
model.compile(loss='mean_squared_error', optimizer='adam', metrics=['accuracy'])#mean_squared_error   adam   RMSprop ADAgrad
print(model.summary())
history = model.fit(X_Train, Y_Train, validation_data=(X_Valid, Y_Valid),verbose=2, epochs=100)
...
model.add(GRU(32,input_shape=(3,8), return_sequences=False,activation='tanh'))
model.add(Dropout(0.3))
model.add(Dense(1, activation='sigmoid'))
